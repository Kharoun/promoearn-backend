        ...
        batch.map(async (user) => {
          ...
          try {
            const sendResult = await resend.emails.send({
              ...
              to:      user.email,
              subject,
              html:    buildEmailHtml(personalBody, imageUrl || null, imagePosition || "top"),
            });
...
// ── Email HTML builder ────────────────────────────────────────────────────────
function buildEmailHtml(bodyText, imageUrl = null, imagePosition = "top") {
  const escaped = bodyText
    .replace(/&/g, "&amp;")
    .replace(/</g, "&lt;")
    .replace(/>/g, "&gt;");

  // If it's a base64 data URL (uploaded file), embed inline.
